...
try {
    ...
    if (!$recipient) {
        throw new Exception("No matching message found for ID: " . $api_message_id, 404);
    }

    // Prevent processing the same DLR twice (e.g., from 'sent' to 'delivered')
    if ($recipient['status'] === 'Delivered' || $recipient['status'] === 'Failed') {
         throw new Exception("DLR for message ID " . $api_message_id . " has already been processed.", 200);
    }
    ...
    $error_code = ($e->getCode() >= 100 && $e->getCode() < 600) ? $e->getCode() : 500;

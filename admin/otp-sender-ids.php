...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['manual_add_sender_id'])) {
    ...
    if ($user_id > 0 && !empty($sender_id) && !empty($sample_message) && in_array($status, ['approved', 'pending', 'rejected'])) {
        $stmt = $conn->prepare("INSERT INTO sender_ids (user_id, sender_id, sample_message, status, type) VALUES (?, ?, ?, ?, 'otp')");
        $stmt->bind_param("isss", $user_id, $sender_id, $sample_message, $status);
        if ($stmt->execute()) {
            $success = "OTP Sender ID manually added successfully.";
        } else {
            $errors[] = "Failed to add OTP Sender ID. It might already exist for this user.";
        }
        $stmt->close();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_status'])) {
    ...
    if (in_array($new_status, ['approved', 'rejected'])) {
        $stmt = $conn->prepare("UPDATE sender_ids SET status = ? WHERE id = ? AND type = 'otp'");
        $stmt->bind_param("si", $new_status, $sender_id_record_id);
        if ($stmt->execute()) {
            $success = "OTP Sender ID status updated to '$new_status'.";
        } else {
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title m-0">All OTP Sender ID Submissions (Termii)</h3>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#manualAddModal">
            <i class="fas fa-plus"></i> Manually Add OTP Sender ID
        </button>
    </div>
    <div class="card-body">
        <div class="row mb-3">
...
<div class="modal fade" id="manualAddModal" tabindex="-1" aria-labelledby="manualAddModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="otp-sender-ids.php" method="POST">
                <div class="modal-header">
                    <h5 class="modal-title" id="manualAddModalLabel">Manually Add OTP Sender ID</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="manual_add_sender_id" class="btn btn-primary">Add OTP Sender ID</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchSenderIds() {
        ...
        fetch('ajax/search_otp_sender_ids.php', {
            method: 'POST',
            body: formData
        })
